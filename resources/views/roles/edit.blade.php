<x-adminlte-layout>
    <div>
        <h1 class="my-1 text-3xl font-semibold text-gray-900">{{ __('Edit Role') }}</h1>
    </div>
                    ...
                    <form method="POST" action="{{ route('roles.update', $role) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}"
                                ...
                                required>
                            <p class="mt-1 text-sm text-gray-500">Gunakan huruf kecil, angka, dan tanda hubung (contoh:
                                super-admin)</p>
                            @error('name')
                        ...
                        <div class="mb-4">
                            <label for="display_name" class="block text-sm font-medium text-gray-700">Nama
                                Tampilan</label>
                                        ...
                                        @foreach ($groupPermissions as $permission)
                                            <div class="flex items-center">
                                                <input type="checkbox" name="permissions[]"
                                                    ...
                                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 group-checkbox"
                                                    data-group="{{ $group }}"
                                                    {{ in_array($permission->id, old('permissions', $role->permissions->pluck('id')->toArray())) ? 'checked' : '' }}>
                                                <label for="permission_{{ $permission->id }}"
                                                    class="ml-2 text-sm text-gray-700">{{ $permission->display_name }}</label>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
